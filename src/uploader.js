...
BPTool.Uploader = typeof BPTool.Uploader != "undefined" && BPTool.Uploader ? BPTool.Uploader : function() {
    ...
    '        <div style="position:absolute; top:0px; left:16px;">{fileLabel}</div>' +
    '        <div style="position:absolute; top:0px; right:94px;">{sizeLabel}</div>' +
    '        <div style="position:absolute; top:0px; right:8px;">{removeLabel}</div>' +
    '    </div>' +
    '    <div id="{dzid}" style="position:absolute; top:14px; left:0px; {filesWidth}; {filesHeight}; border:{fileBorder}; background:{fileBackground}; overflow-y:auto;overflow-x:hidden;">' +
    '        {files}' +
    '    </div>' +
    ...
    '    <div style="position:absolute; bottom:0px; left:0px; {filesWidth}; height:29px; border:{footerBorder}; color:{footerColor}; background:{footerBackground}; font:{footerFont};">' +
    ...
    return {
        create: function(containingDiv, initParams) {
            ...
            if (ConfigParams.zipFiles) {
                ServiceList.push({service: "Zipper", version: "1"});
            }
            ...
            return function() {
                ...
                function renderCoreUI() {
                    ...
                    filesHeight = container.offsetHeight ? ("height:" + (container.offsetHeight - 46) + "px") : "bottom:30px";
                    filesWidth  = container.offsetWidth  ? ("width:" + container.offsetWidth + "px") : "right:0px";
